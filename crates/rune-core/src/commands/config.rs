...
pub(super) fn config(context: &mut CommandContext<'_>) -> CommandOutput {
    ...
    match operation {
        ...
        "set" => usage(
            "config",
            "usage: config set history-limit|font-size|theme VALUE",
        ),
        "reset" if context.args.len() == 1 => reset(context),
        "get" => usage("config", "usage: config get history-limit|font-size|theme"),
...
fn get(context: &CommandContext<'_>, key: &str) -> CommandOutput {
    match key {
        ...
        _ => usage(
            "config",
            "unknown key; available keys: history-limit, font-size, theme",
        ),
    }
}
