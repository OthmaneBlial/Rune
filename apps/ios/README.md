...
the fallback. Blink requests are rendered by the source-only timeline surface
when `CSI ?12h` / `CSI ?12l` or DECSCUSR blink modes request them. The terminal
view can switch to a source-only
