...
Bounded one-line `for`/`while`/`until` bodies and `if` branches with optional
`elif`/`else` clauses are normalized into the same planner path as multiline
scripts, with quote-aware control-marker detection.
...
Host-session action tests verify that argument-free `exit`, `newWindow`, and
`pickFolder` commands produce no command output, invalid arguments remain usage
errors, and the C ABI transfers each action exactly once. The source-only Swift
workspace routes those values to tab/window/folder-picker policy; Apple runtime
behavior is not inferred from this test.
