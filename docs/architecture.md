...
inverse controls after the Rust boundary; unsupported terminal controls are
deliberately bounded and not claimed as a complete emulator.
...
`du`, `md5`, `realpath`, `rmdir`, `sha256`, `stat`, `unlink`, `tee`, `tr`, and `xxd` commands, plus
`ln -s`/`readlink` and
...
extensions before writing into the confined destination. ZIP64, compression,
PAX extensions, and broad external compatibility are not claimed.
